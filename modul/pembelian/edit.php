...
<?php include '../../layout/sidebar.php'; ?>
    ...
    <main class="flex-1 p-4 sm:p-6 animate-fade-in">
        <div class="bg-white rounded-xl shadow-lg p-6 sm:p-8">
            <h2 class="text-2xl font-bold text-gray-800 mb-8 border-b pb-4">Edit Transaksi Pembelian Barang</h2>
            ...
            // Ambil data pembelian yang akan diedit
            $id_pembelian = $_GET['id'];
            $query_pembelian = "SELECT * FROM pembelian WHERE id_pembelian = '$id_pembelian'";
            ...
            <form id="formPembelian" class="space-y-8" method="POST" action="proses_edit.php">
                    ...
                    <div class="md:col-span-2 bg-gray-50 p-4 sm:p-6 rounded-lg space-y-4 sm:space-y-6 border border-gray-300">
                        <div class="flex flex-col sm:flex-row gap-4 sm:gap-6">
                            <div class="w-full sm:flex-1">
                                ...
                                <input type="text" id="noFaktur" name="no_invoice_pembelian" value="<?php echo $data_pembelian['no_invoice_pembelian']; ?>" class="mt-1 block w-full rounded-lg border border-gray-300 bg-white px-3 sm:px-4 py-2 focus:border-blue-500 focus:ring-2 focus:ring-blue-200" placeholder="Masukkan No Faktur" required>
                            </div>
                            
                            <div class="w-full sm:flex-1">
                                <label class="block text-sm font-semibold text-gray-700 mb-2">Tanggal Pembelian <span class="text-red-500">*</span></label>
                                <input type="date" id="tanggalPembelian" name="tanggal" value="<?php echo $data_pembelian['tanggal']; ?>" class="mt-1 block w-full rounded-lg border border-gray-300 bg-white px-3 sm:px-4 py-2 focus:border-blue-500 focus:ring-2 focus:ring-blue-200" required>
                            </div>
                        </div>
                        ...
                        <div class="w-full">
                            ...
                            <select id="supplier" name="id_supplier" class="mt-1 block w-full rounded-lg border border-gray-300 bg-white px-3 sm:px-4 py-2 focus:border-blue-500 focus:ring-2 focus:ring-blue-200" required>
                                ...
                                while($row = mysqli_fetch_assoc($result)) {
                                    $selected = ($row['id_supplier'] == $data_pembelian['id_supplier']) ? 'selected' : '';
                    ...
                    <div id="tabelDinamis" class="overflow-x-auto">
                        <table class="w-full">
                            ...
                            <tbody id="detailBarang" class="divide-y divide-gray-300">
                                <?php
                                $query_detail = "SELECT d.*, b.nama_barang, b.kode_barang FROM pembelian_detail d 
                                               JOIN barang b ON d.id_barang = b.id_barang 
                                               WHERE d.id_pembelian = '$id_pembelian'
                                               AND d.status_dihapus = 0";
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                    ...
                    <div class="bg-gray-50 p-6 rounded-lg space-y-4 border border-gray-300">
                        <div class="flex justify-between items-center">
                            ...
                            <span id="subtotal" class="text-lg font-bold text-gray-900">Rp <?php echo number_format($data_pembelian['total_bayar'] / (1 - ($data_pembelian['diskon']/100)), 2, ',', '.'); ?></span>
                        </div>
                        <div class="flex items-center gap-4 bg-white p-4 rounded-lg border border-gray-300">
                            ...
                            <input type="number" id="diskonPersen" name="diskon" min="0" max="100" class="w-24 rounded-lg border border-gray-300 px-3 py-2 focus:border-blue-500 focus:ring-2 focus:ring-blue-200" value="<?php echo $data_pembelian['diskon']; ?>">
                            <span id="diskonNominal" class="text-gray-700 font-medium">Rp <?php echo number_format(($data_pembelian['total_bayar'] / (1 - ($data_pembelian['diskon']/100))) * ($data_pembelian['diskon']/100), 2, ',', '.'); ?></span>
                        </div>
                        <div class="flex justify-between items-center pt-4 border-t border-gray-300">
                            ...
                            <span id="totalAkhir" class="text-2xl font-bold text-blue-600">Rp <?php echo number_format($data_pembelian['total_bayar'], 2, ',', '.'); ?></span>
                            <input type="hidden" name="total_bayar" id="totalBayarInput" value="<?php echo $data_pembelian['total_bayar']; ?>">
                        </div>
                    </div>
                </div>
                ...
                <div class="flex justify-end gap-4 pt-6 border-t border-gray-300">
                    ...
                    <button type="submit" class="px-8 py-3 bg-blue-600 text-white rounded-lg font-semibold hover:bg-blue-700 transition-colors duration-200 focus:outline-none focus:ring-2 focus:ring-blue-500">
                        Simpan Perubahan
                    </button>
                </div>
            </form>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            let barangCounter = <?php echo $counter; ?>;
            ...
            const form = document.getElementById('formPembelian');
                form.addEventListener('submit', function(e) {
                    e.preventDefault();
                    if (confirm('Apakah Anda yakin ingin menyimpan perubahan pembelian ini?')) {
            ...
            document.querySelectorAll('.harga').forEach(hargaInput => {
                hitungSubtotal(hargaInput);
            });
        });
