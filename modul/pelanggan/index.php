...
<?php include '../../layout/sidebar.php'; ?>
          ...
          <main class="flex-1 p-4 sm:p-6 animate-fade-in">
              <div class="bg-white rounded-lg shadow-md p-4 sm:p-6 animate-slide-in-up">
                ...
                <!-- Pesan Data Sukses -->
                <?php
                    if(isset($_SESSION['pelanggan_sukses'])) { ?>
                    <div id="alert-message" class="mb-4 bg-green-100 border-l-4 border-green-500 text-green-700 p-4 rounded animate-fade-in-down flex justify-between items-center" role="alert">
                        <p class="font-medium"><?php echo $_SESSION['pelanggan_sukses']; ?></p>
                ...
                <?php 
                unset($_SESSION['pelanggan_sukses']);
                } 
                ?>  
                  ...
                  <div class="mb-4 animate-fade-in">
                      <div class="relative">
                          <input type="text" id="searchInput" class="w-full pl-10 pr-4 py-2 border rounded-lg focus:outline-none focus:border-blue-500 transition-all duration-300" placeholder="Cari pelanggan...">
                          ...
                          <table id="penggunaTable" class="min-w-full border border-gray-200 divide-y divide-gray-200">
                              <thead class="bg-gray-50">
                                  <tr>
                                      ...
                                      <th class="px-3 py-3 sm:px-6 text-xs cursor-pointer hover:bg-gray-100 border-r transition-colors duration-200" data-column="no_hp_pelanggan">
                                          <div class="flex items-center">
                                              No HP
                                              <svg class="w-3 h-3 ml-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 16V4m0 0L3 8m4-4l4 4m-4 4v12m0 0l4-4m-4 4l-4-4"/>
                                      ...
                                      <th class="px-3 py-3 sm:px-6 text-xs cursor-pointer hover:bg-gray-100 border-r transition-colors duration-200" data-column="alamat_pelanggan">
                                          <div class="flex items-center">
                                              Alamat
                                              <svg class="w-3 h-3 ml-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 16V4m0 0L3 8m4-4l4 4m-4 4v12m0 0l4-4m-4 4l-4-4"/>
          ...
          let currentPage = 1;
          let itemsPerPage = 10;
          let sortColumn = 'nama_pelanggan';
          let sortDirection = 'asc';
          
          async function fetchData() {
              try {
                  const response = await fetch('get_pelanggan.php');
          ...
          function renderTable(data = tableData) {
              ...
              paginatedData.forEach((row, index) => {
                  ...
                  tr.innerHTML = `
                      ...
                      <td class="px-6 py-4 border-b border-r border-gray-200">${row.no_hp_pelanggan}</td>
                      <td class="px-6 py-4 border-b border-r border-gray-200">${row.alamat_pelanggan}</td>
                      <td class="px-6 py-4 border-b border-gray-200">
                          <div class="flex justify-center space-x-3">
                              <button onclick="detailPelanggan(${row.id})" class="text-red-500 hover:text-red-700 p-1.5 rounded-full hover:bg-red-100">
                                <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/>
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/>
                                </svg>
                              <button>
                              <button onclick="editPelanggan(${row.id})" class="text-yellow-500 hover:text-yellow-700 p-1.5 rounded-full hover:bg-yellow-100">
                                  <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                      <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/>
                                  </svg>
                              </button>
                              <button onclick="hapusPelanggan(${row.id})" class="text-red-500 hover:text-red-700 p-1.5 rounded-full hover:bg-red-100">
                                  <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
